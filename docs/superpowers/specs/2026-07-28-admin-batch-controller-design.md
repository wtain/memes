...
1. `batch_name` validated against `batch.registry.BATCH_REGISTRY` — `404` if not a recognized name.
2. **Deliberate exception to the usual `get_async_db` convention:** this endpoint calls
   `repo.create_run(kind=registry[batch_name]["kind"], trigger="manual")` on its own session (still
...
class RunStatusResponse(BaseModel):
    run_id: str
    batch_name: str      # mapped from BatchRun.kind via the registry's reverse lookup
